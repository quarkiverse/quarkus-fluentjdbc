...
The `RecordMapper` maps `ResultSets` into Java Records. It will also automatically map the database column name into camel case in Java.
...
If you need to create dynamic queries where certain parameters can be empty, then you use `DynamicQuery` as follows:
...
`calories` and `carbohydrates` is a shorthand for: `calories = :calories` and `carbohydrates = :carbohydrates` respectively.
